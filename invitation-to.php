...
$invited = $inv_result[invited_userid];

mysql_query("UPDATE freelancer_mmv_member_invitation SET readstatus='1' WHERE invitation_id='$invid'");

$query = mysql_query("SELECT * FROM freelancer_mmv_member_master WHERE member_id='$invited'");
...
if(isset($_POST[submitrating])){
	...
	if($query3==1){
		echo "<script>window.location='invitation-to.php?status=posted'</script>";
	}
	echo "<script>window.location='invitation-to.php?id=$invid'</script>";	
}
