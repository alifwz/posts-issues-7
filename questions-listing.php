...
<?php if($filter_res[title]!=""){ echo '<h3 align="center" style="font-size:16px">'.$filter_res[title].'</h3>'; } else { echo '&nbsp'; } ?>
	...
	<?php
	if($countryid !=""){
		$banner_que = mysql_query("SELECT * from freelancer_mmv_questions where countryid='$countryid' $catfilter AND status='1' ORDER BY lastseen DESC");		
	} else {		
		$banner_que = mysql_query("SELECT * from freelancer_mmv_questions where status='1' $catfilter ORDER BY lastseen DESC");
	}
	...
	?>	
		<div class="listing-row" onClick="location.href='listing-detail.php?id=<?php echo $banner_result[id] ?>'">			
		...
		<?php } else { ?>
		<div class="listing-row" onClick="location.href='postajobedit.php?id=<?php echo $banner_result[id] ?>'">			
			<h3><?php echo $banner_result[title] ?></h3>
